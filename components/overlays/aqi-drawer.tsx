      ...
      <Animated.View style={{ position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: '#FFFFFF', borderTopLeftRadius: 32, borderTopRightRadius: 32, borderCurve: 'continuous', boxShadow: '0 -10px 30px rgba(0,0,0,0.16)', transform: [{ translateY: progress.interpolate({ inputRange: [0, 1], outputRange: [520, 0] }) }] }}>
        <View style={{ padding: 24, paddingBottom: Math.max(insets.bottom, 32) }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <Text style={{ color: '#1E293B', fontSize: 18, fontWeight: '700' }}>即時空氣品質</Text>
          ...
          <View style={{ marginBottom: 24, paddingBottom: 24, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' }}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 12 }}>
              <Text style={{ color: aqiStatus.indicatorColor, fontSize: 48, lineHeight: 48, fontWeight: '600', letterSpacing: -2, fontVariant: ['tabular-nums'] }}>{data.aqi.value}</Text>
              <View style={{ paddingBottom: 4 }}><Text style={{ color: aqiStatus.indicatorColor, fontSize: 15, fontWeight: '500' }}>{aqiStatus.label}</Text><Text style={{ color: '#94A3B8', fontSize: 11, marginTop: 2 }}>AQI 指數</Text></View>
            </View>
            <Text style={{ color: '#334155', fontSize: 15, lineHeight: 22, fontWeight: '600', marginTop: 12 }}>{aqiStatus.phrase}</Text>
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <Text style={{ color: '#64748B', fontSize: 14, lineHeight: 20, fontWeight: '500' }}>{stationName}</Text>
